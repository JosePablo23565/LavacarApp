  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // Si no existe perfil o falta nombre o teléfono, redirigir a completar
        if (!perfil || !perfil.nombre || !perfil.telefono) {
          // Guardar datos básicos si ya tenemos nombre de Google
          if (userNombre && !perfil) {
            await supabase.from('perfiles').insert([
              { id: userId, nombre: userNombre, telefono: '', email: userEmail }
            ])
          }
        ...
        // Si el perfil está completo, ir al home
        navigate('/')
      } catch (err) {
